...
from net import Restormer_Encoder, Restormer_Decoder, BaseFeatureExtraction, DetailFeatureExtraction, \
    feature_weight_map, feature_enhancement
import os
import numpy as np
...
ckpt_path=r"models/MFDFuse_06-09-09-25.pth"
for dataset_name in ["TNO","RoadScene","MSRS","M3FD"]:
    ...
    # BaseFuseLayer = nn.DataParallel(BaseFeatureExtraction(dim=64, ffn_expansion_factor=2)).to(device)
    ...
    FeatureWeightMap.load_state_dict(torch.load(ckpt_path)['FeatureWeightMap'])
    FeatureEnhancement.load_state_dict(torch.load(ckpt_path)['FeatureEnhancement'])
    #预测模式
    Encoder.eval()
    Decoder.eval()
    BaseFuseLayer.eval()
    DetailFuseLayer.eval()
    FeatureWeightMap.eval()
    FeatureEnhancement.eval()
    ...
    with torch.no_grad():
        for img_name in os.listdir(os.path.join(test_folder,"ir")):
            ...
            # data_Fuse, _ = Decoder(data_VIS, feature_F_B+low_frequency_F, feature_F_D+high_frequency_F, feature_vis, feature_ir)
            # zero_feat = torch.zeros_like(feature_V_D)
            # zero_vis = torch.zeros_like(data_VIS)
            # data_Fuse, _ = Decoder(zero_vis, feature_V_D, zero_feat, None,
            #                        None)
            data_Fuse, _ = Decoder(data_VIS, feature_F_B + low_frequency_F, feature_F_D + high_frequency_F,
    ...
    for img_name in os.listdir(os.path.join(ori_img_folder,"ir")):
            ...
            #每个结果写入文本
            result = np.array([Evaluator.EN(fi), Evaluator.SD(fi)
                                        , Evaluator.SF(fi), Evaluator.MI(fi, ir, vi)
                                        , Evaluator.SCD(fi, ir, vi), Evaluator.VIFF(fi, ir, vi)
                                        , Evaluator.Qabf(fi, ir, vi), Evaluator.SSIM(fi, ir, vi)])
            # with open('./index', 'a', encoding='utf-8') as f:
            #     # 写入文件
            #     f.write(str(img_name) + str(result) + '\n')
